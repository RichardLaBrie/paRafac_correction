...
#'@param nm1 is the desired wavelength. Default is 440
#'@param skip is the number of lines to skip before getting the header in CDOM files


#'@export

Abs.val <- function(data.file = "data", FileSelect = F, skip = 1, nm = 440)
{
if(!FileSelect) file.dir = list.files("./data/CDOM")
if(FileSelect) file.dir = choose.files("./data/CDOM")

CDOM440 = matrix(0, nrow = length(file.dir), ncol=1)
rownames(CDOM440) = file.dir
